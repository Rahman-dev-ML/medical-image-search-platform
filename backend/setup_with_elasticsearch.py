...
def install_compatible_packages():
    """Install compatible Elasticsearch packages"""
    print("🔄 Installing compatible Elasticsearch packages...")
    try:
        # Uninstall existing packages
        subprocess.check_call([sys.executable, '-m', 'pip', 'uninstall', '-y', 'elasticsearch', 'django-elasticsearch-dsl', 'django-elasticsearch-dsl-drf'])
        
        # Install compatible versions
        subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'elasticsearch==7.17.9'])
        subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'django-elasticsearch-dsl==7.2.2'])
        subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'django-elasticsearch-dsl-drf==0.22.5'])
        
        print("✅ Compatible Elasticsearch packages installed")
        return True
...
def setup_elasticsearch():
    ...
    if not check_elasticsearch():
        ...
        print("1. Download Elasticsearch 7.17.x from: https://www.elastic.co/downloads/past-releases")
...
def main():
    """Main setup function"""
    print("Medical Image Search Platform - Complete Setup with Elasticsearch")
    print("=" * 70)
    ...
    if success:
        ...
        print(f"   2. Basic search: http://127.0.0.1:8000/api/xrays/?search=pneumonia")
        print(f"   3. Description search: http://127.0.0.1:8000/api/xrays/?search=opacity")
        
        if es_success:
            print(f"\n🚀 Advanced Elasticsearch features:")
            print(f"   • Fuzzy search: http://127.0.0.1:8000/api/elasticsearch/search/?q=pnumonia")
            print(f"   • Multi-field: http://127.0.0.1:8000/api/elasticsearch/search/?q=lung&body_part=Chest")
            print(f"   • Suggestions: http://127.0.0.1:8000/api/elasticsearch/suggestions/?field=diagnosis&text=pneu")
            print(f"   • Analytics: http://127.0.0.1:8000/api/elasticsearch/analytics/")
